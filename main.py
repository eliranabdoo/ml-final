...
def main():
    ...
    meta_results = pd.DataFrame(data={'db_name': X.index, 'y_true': y, 'y_pred': y_pred}).set_index('db_name')
    meta_results.to_csv(os.path.join('/kaggle/working', 'meta-results.csv'))
    meta_results.describe().to_csv(os.path.join('/kaggle/working', 'meta-results-describe.csv'))

    dbs_we_won = list(meta_results[meta_results.y_true == 1].index)
    dbs_we_lost = list(meta_results[meta_results.y_true == -1].index)

    won_dbs_lengths = [len(X) for (db_name, X, y) in raw_dbs if db_name.split('.')[0] in dbs_we_won]
    lost_dbs_lengths = [len(X) for (db_name, X, y) in raw_dbs if db_name.split('.')[0] in dbs_we_lost]
